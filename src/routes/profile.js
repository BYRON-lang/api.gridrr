...
const { getProfileByUserId, upsertProfile, getProfileByDisplayName } = require('../models/profile');
...
// Get profile by display name (public)
router.get('/displayname/:displayName', async (req, res) => {
  try {
    ...
    const profile = await getProfileByDisplayName(displayName, currentUserId);
    if (!profile) {
      return res.status(404).json({ error: 'Profile not found' });
    }
    res.json(profile);
  } catch (error) {
    console.error('Get profile by display name error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
